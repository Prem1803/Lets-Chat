...
let SERVER_URL:string = process.env.REACT_APP_FRONTEND || '';
const connectWithSocketServer = (userDetails: UserDetails) => {
    ...
    socket.on("friend-invitations", (data) => {
        store.dispatch<any>(setPendingInvitations(data) as any);
    })


    socket.on("friends-list", (data) => {
        store.dispatch<any>(setFriends(data) as any);
    });

    socket.on("online-users", (data) => {
        store.dispatch<any>(setOnlineUsers(data) as any);
    });


    socket.on("direct-chat-history", (data) => {
        // store.dispatch<any>(setMessages(data.messages) as any);
        ...
        if (isActive) {
            store.dispatch<any>(setMessages(messages) as any);
        }
    ...
    socket.on("notify-typing", (data) => {
        ...
        store.dispatch<any>(setTyping({typing: data.typing, userId: data.senderUserId}) as any);
    });


    socket.on("call-request", (data) => {
        console.log(data);
        store.dispatch<any>(setCallRequest(data) as any);
    })


    socket.on("notify-chat-left", () => {
        store.dispatch<any>(clearVideoChat("User left the chat...!") as any);
    })
};
...
const callRequest = (data: {
    ...
    const peerConnection = () => {
        ...
        peer.on("stream", (stream) => {
            ...
            store.dispatch<any>(setRemoteStream(stream) as any);
        });

        socket.on("call-response", (data) => {
            const status = data.accepted ? "accepted" : "rejected";
            store.dispatch<any>(setCallStatus(status) as any);

            if (data.accepted && data.signal) {
                console.log("ACCEPTED", data.signal);
                store.dispatch<any>(setOtherUserId(data.otherUserId) as any);
    ...
    getLocalStreamPreview(data.audioOnly, () => {
        peerConnection();
        store.dispatch<any>(setCallStatus("ringing") as any)
        store.dispatch<any>(setAudioOnly(data.audioOnly) as any);
    })
};
...
const callResponse = (data: {
    ...
    if (!data.accepted) {
        return store.dispatch<any>(setCallRequest(null) as any);
    }
    ...
    getLocalStreamPreview(data.audioOnly, () => {
        peerConnection();
        store.dispatch<any>(setCallRequest(null) as any);
        store.dispatch<any>(setAudioOnly(data.audioOnly) as any);
    });
